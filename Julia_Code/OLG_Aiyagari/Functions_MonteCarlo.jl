...
@with_kw struct Model_Panel
    ...
    t_mat = Array{Float64}(undef,N_Panel)         ; 
    # Random number seed 
...
###################################################################
###################################################################
## Functions for simulation and timing 


###################################################################
###################################################################
## Simulate the panel 
function Simulate_Panel_Timed(M::Model,M_P::Model_Panel)
    ## Unpack relevant objects from model 
    @unpack p, MP_ϵ, a_grid, a_grid_fine, a_max, n_ϵ, Γ, G_ap_fine, G_c_fine, n_a_fine, θ_a_f = M
    @unpack a_min, Surv_Pr, Max_Age = p
    @unpack N_Panel, T_Panel, T_Simul = M_P
    ...
    for i = 1:N_Panel 
        ...
        ## Draw initial conditions from stationary distribution (cross-section)
        # Draw ϵ[i] and h[i] from (stationary) marginal distribution 
        ϵ_vec = rand(Categorical( sum(Γ,dims=(1,3))[:] )) ; 
        h_vec = rand(Categorical( sum(Γ,dims=(1,2))[:] )) ; 
        # Draw a[i] from conditional distribution 
        if h_vec==1 
            a_vec = b ; 
        else             
            a_vec  = a_grid_fine[ rand( Categorical( Γ[:,ϵ_vec,h_vec]/sum(Γ[:,ϵ_vec,h_vec]) ) ) ]            ;     
        end 
    ...
    for t = 1:T_Simul
        ...
        Death_Draw = rand(Categorical( [ Surv_Pr[h_vec] , 1-Surv_Pr[h_vec] ] )) ; 
        ...
        if Death_Draw==2 # Agent dies
            ...
            if a_min<a_vec<a_max 
                ...
                a_vec  = ω*G_ap_fine[i_lo,ϵ_vec,h_vec] + (1-ω)*G_ap_fine[i_lo+1,ϵ_vec,h_vec]    ;
            elseif  a_vec==a_max 
                a_vec  = G_ap_fine[end,ϵ_vec,h_vec] ;
            elseif  a_vec==a_min 
                a_vec  = G_ap_fine[ 1 ,ϵ_vec,h_vec] ;
            else 
                error("Error in simulation: assets not working")
            end 
        ...
        if t>=T_Simul-(T_Panel-1)
            ...
            i_lo      = min(n_a_fine-1, Grid_Inv(a_vec,n_a_fine,θ_a_f,a_min,a_max) ) ;
            ω         = min(1,max(0,(a_vec-a_grid_fine[i_lo])/(a_grid_fine[i_lo+1]-a_grid_fine[i_lo]))) ; 
            c_vec  = ω*G_c_fine[i_lo,ϵ_vec,h_vec] + (1-ω)*G_c_fine[i_lo+1,ϵ_vec,h_vec] ;
            elseif  a_min==a_vec
            c_vec  = G_c_fine[1,ϵ_vec,h_vec] ; 
            else 
            c_vec  = G_c_fine[end,ϵ_vec,h_vec] ;
            end 

            ## Save results in panel 
            M_P.a_mat[i,t-(T_Simul-T_Panel)] = a_vec ; # Assets 
            M_P.ϵ_mat[i,t-(T_Simul-T_Panel)] = ϵ_vec ; # Labor efficiency
            M_P.h_mat[i,t-(T_Simul-T_Panel)] = h_vec ; # Age
...
###################################################################
## Simulate the cohort  
function Simulate_Cohort_Timed(M::Model,M_C::Model_Cohort,age_0::Int)
    ## Unpack relevant objects from model 
    @unpack p, MP_ϵ, a_grid, a_grid_fine, a_max, n_ϵ, Γ, G_ap_fine, G_c_fine, n_a_fine, θ_a_f = M
    @unpack a_min, Surv_Pr, Max_Age = p
    @unpack N_Panel = M_C
    ...
    aa, M_C.t_mat[i] = @timed begin 
        ...
        ## Draw initial conditions from stationary distribution (cross-section)
        # Draw ϵ[i] and ζ[i] from (stationary) marginal distribution 
        ϵ_mat[i,1] = rand(Categorical( sum(Γ[:,:,age_0]/sum(Γ[:,:,age_0]),dims=(1,3))[:] )) ; 
        h_mat[i,1] = 1 ; 
        # Draw a[i] from conditional distribution 
        if age_0==1 
            a_mat[i,1] .= b ; 
            c_mat[i,1] .= G_c_fine[b_ind,med_ϵ,1] ;
        else 
            Γ_aux       = Γ[:,ϵ_mat[i,1],age_0]./sum(Γ[:,ϵ_mat[i,1],age_0])   ; 
            a_ind       = rand(Categorical(Γ_aux))              ;
            a_mat[i,1]  = a_grid_fine[ a_ind ]                  ;     
