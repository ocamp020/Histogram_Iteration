...
% Functions for Monte-Carlo simulation 
    ...
    classdef Functions_MonteCarlo
        methods(Static)
            ...
            % Inspired from Inverse transform sampling for Panel Simulation from 
            % Joshua Stough (2022). 
            ...
            end % End of inverse transform sampling
            ...
            function [Output1, Output2, Output3, Output4] = Simulate_Cohort(ModelStructure, PanelModel_Structure, a_vec, eps_vec, zeta_vec, c_vec,c_flag)
                ...
                for i=1:N_Panel
                    ...
                    elseif a_vec(i)==a_max
                        a_vec(i) = G_ap_fine(end, eps_vec(i), zeta_vec(i));
                    elseif a_vec(i)==a_min
                        a_vec(i) = G_ap_fine(1, eps_vec(i), zeta_vec(i) );
                    else
                    ...
                    eps_vec(i)  = rand(pdfrnd(Gamma_eps(eps_vec(i)  ,:), 1 ));
                    zeta_vec(i) = rand(pdfrnd(Gamma_zeta(zeta_vec(i),:), 1 ));
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            ...
            % Panel Simulation
            function ModelStructure_Output = Simulate_Panel(ModelStructure,PanelModel_Structure, Seed_Flag) % 
                
                % ModelStructure is the input structure
                % PanelModel_Structure is the structure of the panel parameters
                % Seed_Flag is a Boolean variable
                ...
                n_cut_fine = M.n_cut_fine;

                N_Panel = PM.N_Panel;
                T_Panel = PM.T_Panel;
                T_Simul = PM.T_Simul;
                Simul_tol = PM.Simul_tol;
                ...
                a_panel = zeros(N_Panel, T_Panel);
                c_vec = zeros(N_Panel,1);
                eps_vec = zeros(N_Panel,1);
                eps_panel = zeros(N_Panel, T_Panel);
                zeta_vec = zeros(N_Panel,1);
                zeta_panel = zeros(N_Panel, T_Panel);
